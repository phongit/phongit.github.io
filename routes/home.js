...
router.post('/sendchat', (req, res)=>{
    ...
    var tokenkey = jwt.verify(token, "cookiewebchat");
    var taikhoan = tokenkey.userName;
    var sql = "insert into tinnhan(nguoiGui, nguoiNhan, noiDung, trangThai, thoiGian) values(?,?,?,0,NOW())";
    conn.query(sql, [taikhoan, req.body.nguoiNhan, req.body.noiDung], (err, result)=>{
        if(err){
